    ...
    private List<Transform> getAvailableDirs(Transform position, float tileWidth, float tileHeight)
    {
        ...
        for (int i = -1; i < 2; i++)
        {
            for (int j = -1; j < 2; j++)
            {
                if (((i == 0) || (j == 0)) && !((j == 0) && (i == 0)))
                {
                    if (Physics2D.OverlapPointNonAlloc(new Vector2(transform.position.x + (i * tileWidth), transform.position.y + (j * tileHeight)), coll, LayerMask.GetMask("nonwalkable", "bomb")) == 0)
                    {
